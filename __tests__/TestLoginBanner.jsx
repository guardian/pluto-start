...
describe("LoginBanner", ()=>{
    ...
    it("should display a welcome message if there is content", (done)=>{
        ...
        const rendered = shallow(<LoginBanner  clientId="sdffsfs" resource="fsdfdsfs" redirectUri="fdsfdsfds" oAuthUri="fdsfdsfds"/>);
        setTimeout(()=>{
            ...
            try {
                ...
                expect(rendered.find(".welcome").find("p.smaller").hasClass("warning")).toBeFalsy();
                done()
            } catch(err){
                done.fail(err);
            }
    ...
    it("should display a warning if expiryWarning is set", (done)=>{
        ...
        const rendered = shallow(<LoginBanner clientId="fdsfsdfsd" resource="fdsfds" redirectUri="fdsfdsfs" oAuthUri="fdfdsfsd"/>);
        const fakeProfile = new DecodedProfile({"exp": fakeExpTime,"sub":"someuser","aud":"someapp"});
        rendered.instance().setState({expiryWarning: true, loginData: fakeProfile}, ()=>{
            expect(rendered.find(".welcome").text()).toContain("Your login expires soon!");
            expect(rendered.find(".welcome").find("p.smaller").hasClass("warning")).toBeTruthy();
            done();
        });
    });
})
...
describe("LoginBanner.checkExpiryHandler", ()=>{
    it("should set expiryWarning if the expiry time is less than 300 seconds in the future", (done)=>{
        ...
        const fakeExpTime = nowTimeSeconds + 150;
        const rendered = shallow(<LoginBanner clientId="sdffsfs" resource="fsdfdsfs" redirectUri="fdsfdsfds" oAuthUri="fdsfdsfds"/>);
        rendered.instance().setStatePromise({loginData: new DecodedProfile({"exp": fakeExpTime,"sub":"someuser","aud":"someapp"})}).then(()=>{
            const completionPromise = rendered.instance().checkExpiryHandler();

            if(completionPromise==null){
                done.fail("expiry handler did not return anything so did not set anything");
            } else {
    ...
    it("should set nothing if expiry time is more than 5 minutes in the future", (done)=>{
        ...
        rendered.instance().setStatePromise({loginData: new DecodedProfile({"exp": fakeExpTime,"sub":"someuser","aud":"someapp"})}).then(()=>{
            ...
            if(completionPromise==null){
                ...
                completionPromise.then(()=>{
                    done.fail("returned a promise so presumably set something when it shouldn't");
                });
            }
        })
    });
