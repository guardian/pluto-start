...
describe("delayedRequest", () => {
  it('should cope with UserBeacon being contacted successfully', async () => {
    ...
    const requestOutput = await delayedRequest('/test/url/segment', 200, 'Token')
    expect(fetch).toHaveBeenCalledWith(
      "/test/url/segment", {"headers": {"Authorization": "Bearer Token", "body": ""}, "method": "PUT"}
    );
  ...
  it('should cope if no response received before time out', async () => {
    ...
    const requestOutput = await delayedRequest('/test/url/segment', 2000, 'Token')
    expect(fetch).toHaveBeenCalledWith(
      "/test/url/segment", {"headers": {"Authorization": "Bearer Token", "body": ""}, "method": "PUT"}
    );
    expect(requestOutput).toBe();
